...
test.serial("calls utils random id", t => {
  ...
  const { id } = ReBars.app(t.context.app);
  t.is(id, "rando");
...
test.serial("throws if element not in document", t => {
  ...
  const error = t.throws(() => ReBars.app(t.context.app));
  t.is(error.message, Msg.messages.noEl());
});
...
test.serial("returns storage", t => {
  const { storage, id } = ReBars.app(t.context.app);
...
test.serial("needs Handlebars in order to run", t => {
  ...
  const error = t.throws(() => ReBars.app(t.context.app));
  t.is(error.message, Msg.messages.noHbs());
});
